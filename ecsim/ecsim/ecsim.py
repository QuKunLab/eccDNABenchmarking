...
class sim_all(object):
    ...
    def __init__(self):
        self.parser = argparse.ArgumentParser(prog='ecsim',
                                 ...
                                 epilog='______________________________________________'
                                )
        ### Required Parameters
        ...
        self.parser.add_argument('--simple-ratio', required=False, type=float, help='ratio of simple DNA to simulate. (default is calculate from the template)')
        self.parser.add_argument('--simple-template', required=False, type=str, help='simple template (format: chr start end length id) to simulate from. (alternative)')
        self.parser.add_argument('--chimeric-template', required=False, type=str, help='chimeric template (format: chr start end length id) to simulate from. (alternative)')
    ...
    def simulate(self): 
        seq = seqsim(sample = self.args.sample, 
                     reference = self.args.reference, 
                     path = self.args.path, 
                     linear_number = self.args.linear_number, 
                     circular_number = self.args.circular_number, 
                     seed=self.args.seed,
                     simple_ratio = self.args.simple_ratio,
                     simple_template = self.args.simple_template, 
                     chimeric_template = self.args.chimeric_template,
                    )
        
        lib = libsim(sample = self.args.sample,
                     reference = self.args.reference,  
                     path = self.args.path,
                     seed = self.args.seed,
                     meancov = self.args.meancov,
                     amp = self.args.amp,
                    )
