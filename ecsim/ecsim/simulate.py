...
class profile(object):
    '''
    simple_ratio: [0-1]
    simple_profile: bed file of simple eccDNA in bed format 'chr start end length id'
    chimeric_profile: bed file of chimeric eccDNA in bed format 'chr start end length id'
    ...
    def __init__(self,
        ...
        self.chimeric_profile = chimeric_profile
        ...
        if not self.simple_ratio:    
            if not self.simple_profile and not self.chimeric_profile:
                self.simple_ratio = 0.05
                self.simple_weight = self.cal_simple_weight()
                self.chimeric_weight = self.cal_chimeric_weight()
                
            elif not self.simple_profile:
                self.simple_ratio = 0
                self.chimeric_weight = self.cal_chimeric_weight()
                
            elif not self.chimeric_profile:
                self.simple_ratio = 1
                self.simple_weight = self.cal_simple_weight()
    ...
    def cal_simple_ratio(self):
        '''
        Function to calculate the ratio of simpleDNA molecules from profile
        '''
        simple = pd.read_csv(self.simple_profile,header=None,sep='\t')
        chimeric = pd.read_csv(self.chimeric_profile,header=None,sep='\t')
    ...
    def cal_simple_weight(self):
        '''
        Function to calculate the chr and length weight of simple DNA molecules from profile (if not ).
        '''
        if not self.simple_profile:
            self.simple_profile = join(split(__file__)[0], 'resource', 'profile', 'profile.simple.bed')
        bed = pd.read_csv(self.simple_profile,header=None, sep='\t')
    ...
    def cal_chimeric_weight(self):
        ...
        '''
        if not self.chimeric_profile:
            self.chimeric_profile = join(split(__file__)[0], 'resource', 'profile', 'profile.chimeric.bed')
            ## calculate from profile
        bed = pd.read_csv(self.chimeric_profile,header=None, sep='\t')
...
class seqsim(object):
    ...
    simple_profile: bed file of simple eccDNA in format 'chr start end length id'
    chimeric_profile: bed file of chimeric eccDNA in format 'chr start end length id'
    '''
    def __init__(self, 
                 sample: str,
                 reference: str,
        ...
        self.prof = profile(simple_ratio, simple_profile, chimeric_profile)
    ...
    def merge(self):
        '''
        function to merge simulated simple & comlex file
        '''
        for type_i in range(2):
        ...
        clean_cmd = 'rm {0}* {1}* {2}* {3}*'.format(self.prefix[0][0], 
                                                    self.prefix[0][1],
                                                    self.prefix[1][0],
                                                    self.prefix[1][1])
